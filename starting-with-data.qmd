...
```
Inside the download.file command, the first entry is a character string with the source URL. This source URL downloads a CSV file from 
figshare. The text after the comma ("data/portal_data_joined.csv") is the destination of the file on your local machine. 
...
![Dataframe example @Michonneau2019](./images/data-frame.svg){width=50% #fig-dataframe}
...
a.  What is the class of the object `surveys`?
b.  How many rows and how many columns are in this object?
c.  Which columns are of type `character`?
...
a.  class: data.frame
b.  rows: 34786,  columns: 13
c.  number of rows of type `character`: species_id, sex, genus, species, taxa, plot_type
...
![Numeric indexing in a dataframe](./images/indexing_dataframe.svg){#fig-dataframe-numeric-idx}
...
:::{#exercise-5 .callout-note icon=false}

## Exercise 5 {#exercise-5}

a.  Create a `data.frame` (`surveys_200`) containing only the data in row 200 of the `surveys` dataset.

b.  Remember how `nrow()` gave you the number of rows in a `data.frame`?
  i)   Use that number to pull out just that last row in the data frame.
  ii)  Compare that with what you see as the last row using `tail()` to make sure it's meeting expectations.
  iii) Pull out that last row using `nrow()` instead of the row number.
  iv)  Create a new data frame (`surveys_last`) from that last row.

c. Use `nrow()` to extract the row that is in the middle of the data frame. Store the content of this row in an object named `surveys_middle`.

d. Combine `nrow()` with the `-` notation above to reproduce the behavior of `head(surveys)`, keeping just the first through 6th rows of the surveys dataset.
...
:::{#exercise-6 .callout-note icon=false}

## Exercise 6 {#exercise-6}

a. Change the columns `taxa` and `genus` in the `surveys` data frame into a factor.

b. Using the functions you learned before, can you find out...
  i) How many rabbits were observed?
  ii) How many different genera are in the `genus` column?
